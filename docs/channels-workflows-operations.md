...
`register()` is idempotent **and cheap to call on every boot, even for hundreds of persisted routines**: when a routine's currently-pending action already matches its trigger (interval seconds, or cron expression — the first-run timestamp and stagger offset are not schedule-significant), `register()` is a read-only no-op. Only a missing or mismatched schedule unschedules and reschedules. A consumer that must re-declare all of its routines on every request (the in-memory registry does not persist across requests) no longer needs to build its own hash-gate or fingerprint cache in front of `register()` to avoid thrashing Action Scheduler — the substrate is idempotent on its own.

### Routine generation + watermark fencing

Every `WP_Agent_Routine_Action_Scheduler_Bridge` register call that actually schedules something (i.e. skips the no-op path above) mints a fresh generation (`wp_generate_uuid4()`) and records it, alongside the new chain's first action id as a **watermark**, in ONE non-autoloaded option per routine: `agents_routine_generation_<routine_id>` holds `[ 'generation' => string, 'watermark' => int ]`. Scheduled action args stay purely logical — `array( 'routine_id' => ... )` — so Action Scheduler's exact-match queries keep working.

Fencing needs **no per-action bookkeeping at all**. Action Scheduler action ids are a global, strictly increasing sequence: every action in a chain (the initial insert and every recurrence successor AS spawns) has an id at or above the chain's own watermark, and every action from a superseded chain — including one already claimed by an in-flight worker when the supersede happened — has an id below it. `action_scheduler_before_execute` fires before the queue runner re-checks that the action is still pending; when the fetched action's id is below its routine's current watermark, the bridge cancels it there (firing `agents_routine_action_fenced( $routine_id, $generation, $action_id )` for observability) and the runner's own status check then ignores it, so no recurrence successor is spawned. A routine with no recorded watermark yet (never re-registered this process, or a pre-fix legacy action) is never fenced.

Earlier versions stamped a generation onto every stored action individually, in a per-action option (`agents_routine_action_generation_<action_id>`) written on the `action_scheduler_stored_action` hook. That hook no longer exists: the option was only ever deleted by `cancel()`, and `register()`'s prior unconditional unschedule-and-reschedule never routed through `cancel()`, so the option accumulated without bound — one production site reached 302,623 orphaned rows (10 MB of `wp_options`) against 596 live pending actions. `WP_Agent_Routine_Action_Scheduler_Bridge::purge_legacy_action_generation_options()` is a one-shot cleanup (single `DELETE ... LIKE` query) for rows a pre-fix version already left behind; it runs at most once per site, gated by a persisted marker option checked on `init` rather than a plugin-activation hook (agents-api ships far more often as a Composer-embedded library inside a consumer plugin than as its own activatable plugin).

`WP_Agent_Routine_Registry::current_generation( $id )` exposes the persisted routine generation; `unregister()` deletes the whole per-routine option (generation and watermark together).

### Routine stagger

Routines registered with the same interval would all fire in the same second. `WP_Agent_Routine` accepts `stagger => bool|int` (default `true` for interval routines, `false` for cron expressions, where the expression already *is* the slot; an int is an explicit max window in seconds). `WP_Agent_Routine::stagger_offset()` computes `crc32( 'agents_routine_stagger_' . $id ) % min( interval, max_window )`, capped by `WP_Agent_Routine::MAX_STAGGER_SECONDS` (one hour). The bridge adds the offset to the first-run timestamp. The offset depends only on the routine id, so re-registration always lands the routine back in the same slot — and, since the offset only affects the first-run timestamp and never the recurrence itself, it never causes an unchanged re-register to be mistaken for a schedule mismatch.

### Routine reconcile

`WP_Agent_Routine_Registry::reconcile( array $opts = [] )` repairs drift between the registry and the Action Scheduler store. For every registered, non-paused routine it checks pending-action coverage by logical identity and enqueues a fresh schedule when coverage is missing; pending routine actions whose logical `routine_id` is not registered (or is durably paused) are unscheduled as orphans. It returns `array( 'enqueued' => [ids], 'removed' => [ids], 'unchanged' => [ids], 'errors' => [id => message] )`; `$opts['dry_run']` reports the same shape without writing. `pending_by_routine()` (the one bulk read reconcile uses) always forces a fresh fetch of the Action Scheduler store rather than serving the request-scoped cache `register()` maintains — reconcile's entire purpose is authoritative drift detection, so trusting a cache populated before the drift happened would defeat the point.

The run is serialized through a `$wpdb`-level compare-and-set lock (`agents_routine_reconcile_lock`; a single `INSERT ... ON DUPLICATE KEY UPDATE` against `wp_options`) that never trusts `get_option()`/the options cache for the decision — only the real DB row. A lock older than five minutes is treated as stale and taken over atomically in the same query, and the lock is always released (a direct `DELETE`, again bypassing the cache) in a `finally`. This closes a production failure mode where a killed request left the lock option cached (e.g. in Redis) with no backing DB row: `add_option()` always failed because the cache said the option existed, the stale check read that same cached value, and `delete_option()` found nothing in the DB to delete — the lock stayed stuck "held" until the cache entry was manually cleared. Reading and writing exclusively through `$wpdb`, with an explicit `wp_cache_delete()` after every write, means a stale cache entry with no backing row can never block acquisition.
