...
### Fixed
- expose safe drain diagnostics

## [0.11.1] - 2026-09-12

### Fixed
- annotate prepared wpdb queries for phpcs
- idempotent register(), watermark fencing, wpdb-level reconcile lock

## [0.11.0] - 2026-09-09

### Added
- ability wake targets alongside agents/chat

## [0.10.1] - 2026-09-09

### Changed
- WP_Agent_Routine_Backend contract; Action Scheduler becomes the default backend

## [0.10.0] - 2026-09-09

### Added
- generation fencing, stagger offsets, and reconcile for the Action Scheduler bridge

### Changed
- align assignment after phpcs:ignore annotation

### Fixed
- clear release preflight lint on agents-api.php and the AS stub

## [0.9.0] - 2026-09-08

### Added
- grant/revoke/list-users agent access abilities over WP_Agent_Access_Store

## [0.8.0] - 2026-08-29

### Added
- add external turn actors
- add structured provider output

### Changed
- fail closed when the conversation loop body throws
- Make generic run-control mutations atomic (workflow cancel/finish race)
- Map task run failure into the run-result envelope error field
- Surface failed partial package adoption as incomplete
- update client tool provider alias contract
- Accept typed chat continuations
- Suspend client tools in default chat
- Preserve stateless JSON-RPC history
- Stream default chat provider deltas
- Resolve chat config per request
- Add principal memory layer vocabulary
- Expose JSON-RPC host projection
- Resolve chat runtime profiles
- Expose resolved host dispatch model context
- Add host provider turn dispatch discovery
- Honor explicit chat session principals
- Remove accidental absolute vendor symlink
- Review follow-up: make streaming run claims atomic
- Review follow-up: harden workflow branch reconciliation
- Review follow-up: bind task actions to stored runs
- Review follow-up: simplify bridge queue locking
- Review follow-up: close workspace symlink races
- Review follow-up: enforce aggregate metadata bounds
- Review follow-up: cover nested observer redaction
- Review follow-up: preserve interactive authorization
- Review follow-up: harden conditional lock release
- Review follow-up: validate artifact import success
- Review follow-up: clarify duplicate artifact identity
- Review follow-up: preserve trusted provenance precedence
- Review follow-up: make tool terminal delivery atomic
- enforce exact-once run claim on JSON-RPC chat streaming
- bind branch reconcile to stored state and fail closed on lock loss
- enforce exact-once terminal semantics for runtime-tool lifecycle
- bind conversation session owner to authenticated principal and enforce workspace boundary
- authorize task run actions against stored owner, not client claims
- serialize bridge queue read-modify-write under a lock
- reject leaf symlinks in safe execution workspace file I/O
- bound preserved citation metadata in tool result truncator
- redact workflow run metadata for non-manager readers
- make CPT conversation lock release value-conditional
- enforce autonomous deny-by-default ceiling in authorization policy
- fail closed on non-truthy package artifact import results
- reject duplicate artifact identities in agent packages
- make importer-derived agent provenance authoritative
- Document successful pre-tool completion
- Remove generated reviewer artifact
- Add request-scoped workflow control
- Checkpoint request-scoped controller candidate
- bump actions/checkout
- bootstrap WordPress database host parsing

### Fixed
- Fix reconcile lock expiry during aggregation
- retry branch reconciliation without re-executing effects
- make workflow fanout admission atomic
- finalize workflow run-control recovery
- harden workflow run-control recovery
- normalize workflow run-control contention
- expose all public classes through Composer
- expose package classes through Composer
- preserve normalized pending mediation requests
- preserve structured output shapes
- emit provider-safe host tool names
- unify canonical chat authorization
- Fix principal-owned default chat sessions
- preserve conversation ownership REST statuses
- make bootstrap glob discovery portable
- serialize chat run state mutations

## [0.7.0] - 2026-07-21

### Changed
- Remove legacy workflow drain compatibility (by Chris Huber)

## [0.6.1] - 2026-07-21

### Changed
- Document workflow run awaiting
- Use deterministic Docs Agent completion
- Pin exact completion report prompt
- Execute accepted Docs Agent helpers
- Pin stable Docs Agent provenance
- Adopt Docs Agent completion contract
- Hydrate Docs Agent verification dependencies
- Advance Docs Agent producer chain
- Use Docs Agent #148 and WP Codebox v0.12.23

### Fixed
- Fix workflow foreground drain isolation
- bind queued messages to session owners
- Fix async runner response accounting
- scope chat run control by workspace
- support authoritative parameter bindings
